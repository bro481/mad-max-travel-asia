...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){if(!await getChatGPTUser())return NextResponse.json({error:"Unauthorized"},{status:401});const {id}=await params;if(useLocalProperties()){const item=getLocalProperty(Number(id));return item?NextResponse.json(item):NextResponse.json({error:"Not found"},{status:404});}try{const item=await getProperty(Number(id));return item?NextResponse.json(item):NextResponse.json({error:"Not found"},{status:404});}catch(error){console.error("Failed to load property from database",error);const item=staticPropertyRecords().find((x)=>x.id===Number(id)||x.slug===id);return item?NextResponse.json(item,{headers:{"x-admin-data-source":"static-fallback"}}):NextResponse.json({error:"Not found"},{status:404});}}
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
 if(!await getChatGPTUser())return NextResponse.json({error:"Unauthorized"},{status:401});const {id}=await params;const b=await request.json() as Record<string,unknown>;if(useLocalProperties()){updateLocalProperty(Number(id),b as PropertyRecord);return NextResponse.json({ok:true});}await ensureProperties();
