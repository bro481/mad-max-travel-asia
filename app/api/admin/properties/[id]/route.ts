...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 await env.DB.prepare(`UPDATE properties SET name_zh=?,name_en=?,city=?,area_zh=?,area_en=?,tags=?,images=?,image_categories=?,guests=?,bedrooms=?,beds=?,bathrooms=?,description_zh=?,description_en=?,amenities=?,highlights=?,nearby=?,price_from=?,price_note=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`)
 .bind(String(b.nameZh),String(b.nameEn),String(b.city),String(b.areaZh||""),String(b.areaEn||""),json("tags"),json("images"),JSON.stringify(b.imageCategories||{}),Number(b.guests),Number(b.bedrooms),Number(b.beds),Number(b.bathrooms),String(b.descriptionZh||""),String(b.descriptionEn||""),json("amenities"),json("highlights"),json("nearby"),Number(b.priceFrom),String(b.priceNote||""),String(b.status||"draft"),Number(id)).run();
