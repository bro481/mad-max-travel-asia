...
export default function TravelPhotographyPage() {
  ...
  return <>
    <header>
      ...
      <nav className={menu ? "open" : ""}><a href="/#stays">{t.rooms}</a><ServiceMenu lang={lang} /><a href="/packages">{lang === "zh" ? "省心套餐" : "Packages"}</a><a href="/picks">{lang === "zh" ? "大马特产" : "Malaysia Picks"}</a><a href="/photography">{lang === "zh" ? "旅行攻略" : "Travel Guide"}</a><a className="active-nav" href="/travel-photography">{lang === "zh" ? "大马跟拍" : "Travel Photography"}</a><a href="/about">{t.about}</a><div className="language-switch mobile-language"><button className={lang === "zh" ? "active" : ""} onClick={() => setLang("zh")}>中文</button><i /><button className={lang === "en" ? "active" : ""} onClick={() => setLang("en")}>English</button></div></nav>
    ...
    <footer><a className="logo" href="/"><span className="logo-mark">⌂</span><span><b>MAD MAX</b><small>MALAYSIA STAY</small></span></a><div><a href="/#stays">{t.rooms}</a><a href="/services">{lang === "zh" ? "当地服务" : "Local Services"}</a><a href="/photography">{lang === "zh" ? "旅行攻略" : "Travel Guide"}</a><a href="/#contact">{t.contact}</a></div><small>© 2026 MAD MAX Malaysia Stay</small></footer>
  </>;
}
