...
const meta = {
  ...
  "private-charter": { eyebrow: "PRIVATE CAR · 私人包车", heading: "告诉我你的包车安排", action: "整理我的包车需求", service: "私人包车" },
...
export function InquiryModal({ kind, title, maxGuests = 14, onClose }: InquiryModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    wishes: "", quantity: 1, location: "还在马来西亚", delivery: "住宿期间领取", city: "",
  });
  ...
  const lines = useMemo(() => {
    ...
    if (kind === "private-charter") return [head, `日期：${form.date || "待补充"}`, `人数：${form.adults} 人`, `出发地点：${form.place || "待补充"}`, title ? `已选择路线：${title}` : `安排方式：${form.routeMode}`, `想去的地方：${form.wishes || "待沟通"}`];
  ...
  return <div className="inquiry-layer" role="dialog" aria-modal="true" aria-label={info.heading} onMouseDown={(event) => event.stopPropagation()} onClick={onClose}>
    <div className="inquiry-card" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
      ...
      {generated ? <section className="inquiry-result">
        ...
        {kind === "airport-transfer" && <><label><span>接送方向</span><div className="inquiry-options">{["机场 → 酒店", "酒店 → 机场"].map((x) => <button type="button" className={form.direction === x ? "active" : ""} onClick={() => set("direction", x)} key={x}>{x}</button>)}</div></label><div className="inquiry-grid"><label><span>日期</span><input type="date" value={form.date} onChange={(e) => set("date", e.target.value)} /></label><label><span>航班号（可稍后补）</span><input placeholder="例如 MH123" value={form.flight} onChange={(e) => set("flight", e.target.value)} /></label></div><label><span>接送地点</span><input placeholder="酒店名称 / 地址" value={form.place} onChange={(e) => set("place", e.target.value)} /></label><div className="inquiry-grid"><Stepper label="人数" field="adults" max={maxGuests}/><Stepper label="行李" field="luggage" /></div></>}
        {kind === "accommodation" && <><div className="inquiry-grid"><label><span>入住日期</span><input type="date" value={form.date} onChange={(e) => set("date", e.target.value)} /></label><label><span>退房日期</span><input type="date" value={form.endDate} onChange={(e) => set("endDate", e.target.value)} /></label></div><div className="inquiry-grid"><Stepper label="成人" field="adults" max={maxGuests}/><Stepper label="儿童" field="children" /></div></>}
        {kind === "private-charter" && <><div className="inquiry-grid"><label><span>日期</span><input type="date" value={form.date} onChange={(e) => set("date", e.target.value)} /></label><Stepper label="人数" field="adults" /></div><label><span>出发地点</span><input placeholder="酒店 / 民宿名称或区域" value={form.place} onChange={(e) => set("place", e.target.value)} /></label>{!title && <label><span>想怎么玩</span><div className="inquiry-options">{["推荐路线", "自由安排"].map((x) => <button type="button" className={form.routeMode === x ? "active" : ""} onClick={() => set("routeMode", x)} key={x}>{x}</button>)}</div></label>}</>}
        {kind === "experience" && <><label><span>出行日期</span><input type="date" value={form.date} onChange={(e) => set("date", e.target.value)} /></label><div className="inquiry-grid"><Stepper label="成人" field="adults"/><Stepper label="儿童" field="children"/></div><label><span>住宿地点（可选）</span><input value={form.place} onChange={(e) => set("place", e.target.value)} /></label></>}
