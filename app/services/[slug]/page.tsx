...
export async function generateMetadata({
  ...
  searchParams: Promise<{ service?: string; route?: string }>;
}): Promise<Metadata> {
  ...
  const description = route
    ? [route.duration, route.nodes?.map((node) => node.nameZh || node.title).filter(Boolean).slice(0, 3).join(" · ") || route.stops].filter(Boolean).join(" · ")
  ...
  const url = route && managedItem
    ? `/services/${slug}?service=${managedItem.slug}&route=${routeIndex}`
    : `/services/${slug}${managedItem ? `?service=${managedItem.slug}` : ""}`;
  return {
    title: `${title}｜MAD MAX`,
    description,
