...
export default function PicksPage() {
  ...
  return <>
    <header>
      ...
      <nav className={menu ? "open" : ""}><a href="/#stays">{t.rooms}</a><ServiceMenu lang={lang} /><a href="/packages">{lang === "zh" ? "省心套餐" : "Packages"}</a><a className="active-nav" href="/picks">{lang === "zh" ? "大马特产" : "Malaysia Picks"}</a><a href="/photography">{lang === "zh" ? "旅行攻略" : "Travel Guide"}</a><a href="/about">{t.about}</a><div className="language-switch mobile-language"><button className={lang === "zh" ? "active" : ""} onClick={() => setLang("zh")}>中文</button><i /><button className={lang === "en" ? "active" : ""} onClick={() => setLang("en")}>English</button></div></nav>
